...
export async function onRequestPost(context) {
  ...
  if (!phone || !amount)
    return j({ error: "phone and amount are required" }, 400);
  ...
  if (!conKey || !conSec)
    return j(
      {
        error:
          "MPESA_CONSUMER_KEY and MPESA_CONSUMER_SECRET must be set in Cloudflare env vars",
      },
      500,
    );
  ...
  try {
    ...
    return j({ error: "OAuth request failed: " + e.message }, 502);
  }

  if (!tokenData?.access_token)
    return j(
      { error: "M-Pesa OAuth failed. Check credentials.", detail: tokenData },
      502,
    );
  ...
  try {
    ...
    return j({ error: "STK push failed: " + e.message }, 502);
  }
    ...
    return j(
      {
        error:
          "STK push rejected: " +
          (stkData?.ResponseDescription || stkData?.errorMessage || "Unknown"),
        detail: stkData,
      },
      502,
    );
