/* ============================================================
  ...
  The whole structure is bound to change once production is live
  Majority of the code that is 
============================================================ */

// ── Pricing mirrors DB and services.html ─────────────────────
// ── These are placeholder prices that must be changed during production at all times to match clients rate
...
function _clearDraft() {
  ["joyalty_booking_draft", "joyalty_booking_result"].forEach((k) =>
...
function updatePricePreview() {
  ...
  const { base, packagePrice, extraPrice, total, deposit } = calcPrice(
    svc,
    pkg,
    ext,
  );
  ...
  if (!el) {
    ...
    el.id = "pricePreview";
    el.style.cssText =
      "margin-top:14px;padding:12px 16px;background:rgba(128,128,128,.08);border-radius:8px;border:1px solid rgba(128,128,128,.15);font-size:14px;line-height:1.9;font-family:Quicksand,sans-serif";
...
function goToStep(n) {
  ...
  if (progressLine)
    progressLine.style.width = `${(n / (steps.length - 1)) * 100}%`;
...
function restoreFields(step) {
  ...
  if (step === 1) {
    ...
    setTimeout(updatePricePreview, 80);
  }
  if (step === 2) {
...
function collectStep(step) {
  ...
  if (step === 2) {
    ...
    bookingData.eventLocation = document
      .getElementById("eventLocation")
      .value.trim();
    bookingData.guestCount =
      document.getElementById("guestCount").value || null;
    bookingData.eventDescription = document
      .getElementById("eventDescription")
      .value.trim();
    bookingData.mpesaPhone =
      mpesaPhoneEl?.value.trim() || bookingData.clientPhone;
  }
  ...
  try {
    ...
    if (data.success) {
      mpesaPayBtn.textContent =
        "✅ Check your phone — or wait for sandbox auto-confirm";
...
async function pollPayment(bookingId, attempts) {
  if (attempts > 20) {
    // 20 × 3s = 60s timeout
    alert(
      "Payment confirmation is taking longer than expected. If you received the M-Pesa prompt, please wait a moment and refresh. Contact us at joyaltyphotography254@gmail.com if the issue persists.",
    );
    mpesaPayBtn.disabled = false;
    mpesaPayBtn.textContent = "Retry M-Pesa";
    return;
...
if (lastReceipt) {
  const rb = Object.assign(document.createElement("div"), {
    style:
      "position:fixed;bottom:80px;right:20px;z-index:9999;background:#22c55e;color:#fff;padding:10px 18px;border-radius:50px;font-size:13px;font-family:Quicksand,sans-serif;cursor:pointer;font-weight:600;box-shadow:0 4px 16px rgba(0,0,0,.2)",
