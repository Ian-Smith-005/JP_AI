...
window.confirmLiveName = function () {
  ...
  if (!name) {
    if (liveNameInput) {
      liveNameInput.placeholder = "Enter a name first…";
      liveNameInput.focus();
    }
    return;
  }
  ...
  livePollTimer = setInterval(async () => {
    ...
    try {
      const d = _json(
        await (
          await fetch(
            `/api/live-chat?sessionId=${encodeURIComponent(liveSessionId)}`,
          )
        ).text(),
      );
      if (!d?.messages) return;
      const known = new Set(liveMessages.map((m) => String(m.id)));
      const newMs = d.messages.filter(
...
function startBooking() {
  ...
  bookingFlow.step = 1;
  typeMessage(
    "Sure! What type of session? (wedding, portrait, commercial, event)",
    "bot",
  );
}
async function handleBooking(text) {
  switch (bookingFlow.step) {
    ...
    case 5:
      ...
      typeMessage(
        "Details saved! Head to our Services page to complete your booking and pay the deposit. 👉",
        "bot",
      );
      break;
  }
...
async function sendMessage() {
  ...
  //call the play sound function with "send" or "receive" based on the message type
  playChatSound("send");
}
//Play sound on send/receive (try/catch to avoid issues in unsupported browsers)
